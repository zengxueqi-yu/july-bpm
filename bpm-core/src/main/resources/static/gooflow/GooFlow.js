...
function GooFlow(bgDiv, property) {
    ...
    if (this.$editable) {
        ...
        this.$workArea.on("click", {inthis: this}, function (e) {
            ...
            var uuid = generateUUID(); //This.$max
            var nodeName = "node_" + This.$max;
            if (This.$nowType.indexOf("start") > -1) {
                nodeName = "开始";
            } else if (This.$nowType.indexOf("end") > -1) {
                nodeName = "结束";
            }
            This.addNode(uuid, {name: nodeName, left: X, top: Y, type: This.$nowType});
            This.$max++;
        });
        //划线或改线时用的绑定
...
GooFlow.prototype = {
    ...
    addNode: function (id, json) {
        ...
        if (json.type.indexOf("start") > -1 || json.type.indexOf("end") > -1) {
            json.width = 26;
            json.height = 26;
    ...
    initWorkForNode: function () {
        ...
        this.$workArea.delegate(".GooFlow_item", "mouseup", {inthis: this}, function (e) {
            ...
            if (lineStart && !This.$mpTo.data("p")) {
                var uuid = GooFlow.prototype.generateUUID();
                This.addLine(uuid, {from: lineStart.id, to: this.id, name: ""});
                This.$max++;
            } else {
                if (lineStart) {
        ...
        this.$workArea.delegate(".GooFlow_item > .span", "dblclick", {inthis: this}, function (e) {
            ...
            This.$workArea.parent().one("mousedown", function (e) {    // 仅第一次双击不触发
                ...
                review_type = $("#node_review_type").val();
                executor_roles = get_node_role($("#node_role"));
                notify_roles = get_node_role($("#node_notify_role"));
                This.setName(This.$textArea.data("id"), node_name, "node", executor_roles.scope, executor_roles.text,
                    review_type, executor_user_scope, notify_roles.scope);
                This.$textArea.val("").removeData("id").hide();
                $(".shade,.pop_node").hide();
            });
            ...
            $(".pop_node .btn_sure").click(function (e) {
                ...
                $('#tab tr').each(function (i) {
                    $(this).children('td').each(function (j) {
                        console.log("第" + (i + 1) + "行，第" + (j + 1) + "个td的值：" + $(this).find("input").val() + "。");
                    });
                });
                ...
                var nodeMsg = {
                    "nodeId": node_id,
                    "nodeName": node_name,
                    "nodeApproverMsg": node_role_val,
                    "nodeNotifyMsg": executor_roles,
                    "nodeApproveType": review_type,
                    "nodeFormMsg": ""
                }
                $.ajax({
