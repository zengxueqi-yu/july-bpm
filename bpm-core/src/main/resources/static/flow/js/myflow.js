(function (b) {
        ...
        selectEditor: function (arg) {
            ...
            this.init = function (props, k, div, src, r) {
                ...
                if (typeof arg === 'string') {
                    $.ajax({
                        url: "/flowRuntimeNodeUser/getNodeUserInfo",
                        type: 'post',
                        dataType: 'json',
    ...
    a.init = function (x, r) {
            ...
            if (i.keyCode == 46) {
                ...
                if (j) {
                    if (j.getId().substring(0, 4) == "rect") {
                        a.config.tools.deleteRect.onclick(j.getId(), j.toJson());
                        b(y).trigger("removerect", j)
                    } else {
                        if (j.getId().substring(0, 4) == "path") {
                            a.config.tools.deletePath.onclick(j.getId());
        ...
        b(y).bind("addrect", function (j, c, k) {
            ...
            q[i.getId()] = i
            a.config.tools.addRect.onclick(i.getId(), i.toJson())
        });
        var f = function (i, k, j) {
            var c = new a.path({}, y, k, j);
            g[c.getId()] = c
            a.config.tools.addPath.onclick(c.getId(), c.toJson());
        };
        b(y).bind("addpath", f);
        b(y).data("mod", "point");
        if (a.config.editable) {
